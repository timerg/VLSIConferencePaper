...
Fig 4 a-d shows the post-simulation results for circuit operated under CVM mode.
A simple MOS with proper transconductance(gm) are substituted for nanowire(By experiment beforehand, gm should range from 200n to 20u).
And a voltage signal is input to the gate of the MOS. 20mV is used to emulate the ion effect when measurement solution is added.

Fig. 4a shows the total amplification rate is 6.45db(2.1) for gm: 200n.
This rate increases with the gm in the same degree.
...
Largest input referred noise should happen at gm 200n while the total gain is smallest.
The noise is small enough since its value is ten times smaller than the input signal(20mV).

Fig. 4d gives transient response with an input signal rises in 1ms.
It is to be observed the output signal has a long settling time about 1s.
This is caused by the resistor Rx in Fig. 3.
Its resistance (about 10G ohm in this work) makes trade-off between low speed signal detection and fast responding time.
The resistance will be decided with more specific purpose in the future works.
